...
class FeatEmbedding(nn.Module):
    ...
    def forward(self, feature_matrix: LT, padding: Optional[BT] = None, masked_positions: Optional[LT] = None) -> FT:
        feature_matrix = adv_index(feature_matrix, 'feat_group', self.c_idx)
        # Convert old style to new style ipa features.
        # if g.new_style:
        #     new_feat_matrix = list()
        #     for c_idx, one_feat_group in zip(self.c_idx.unbind(dim=self.group_name), feature_matrix.unbind(dim=self.group_name)):
        ...
        #     new_feat_matrix = torch.cat(new_feat_matrix, dim=-1).refine_names(*feature_matrix.names)
        #     feature_matrix = new_feat_matrix
        feat_emb = embed(self.embed_layer, feature_matrix, self.feat_emb_name)
        ...
        if masked_positions is not None:
            ...
            # feat_emb = self.feat_embeddings(feature_matrix).view(bs, l, -1).transpose(1, 2)  # size: bs x D x l
...
class Encoder(nn.Module):
    ...
    def forward(self, feature_matrix: LT, pos_to_predict: LT, source_padding: BT) -> FT:
        bs = source_padding.size('batch')
        l = source_padding.size('length')
        batch_i = get_range(bs, 1, 0)
        feat_emb = self.feat_embedding(feature_matrix, source_padding, masked_positions=pos_to_predict)
        feat_emb = feat_emb.align_to('batch', 'char_emb', 'length')
